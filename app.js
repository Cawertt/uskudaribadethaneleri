...
// Map Initialization
function initMap() {
    map = L.map('map', { zoomControl: false }).setView(CONFIG.initialView, CONFIG.initialZoom);
    L.control.zoom({ position: 'bottomright' }).addTo(map);
    tileLayer = L.tileLayer(CONFIG.layers.light, { attribution: CONFIG.attribution }).addTo(map);
}
...
function updateTheme() {
    ...
    if (currentTheme === 'satellite') {
        ...
        tileLayer.setUrl(CONFIG.layers.dark);
    } else {
